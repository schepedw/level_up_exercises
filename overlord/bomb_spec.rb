...
describe Bomb do
  ...
  it "should only allow codes of 4 digits" do 
    expect{ Bomb.new("123", "1234")}.to raise_error(ArgumentError)
    expect{ Bomb.new("1234", "12345")}.to raise_error(ArgumentError)
  end

  it "should only allow state change through the accept_code method" do
  ...
  def activate_bomb(bomb)
    bomb.accept_code("1234")
  end

  def deactivate_bomb(bomb)
    bomb.accept_code("0000")
  end
  def try_wrong_code(bomb)
    bomb.accept_code("4249082123049")
  end
